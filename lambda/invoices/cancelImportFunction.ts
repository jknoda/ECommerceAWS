...
import { ApiGatewayManagementApi, ConnectContactLens, DynamoDB } from "aws-sdk"
...
const invoicesWsApiEndpoint = process.env.INVOICE_WSAPI_ENDPOINT!.substring(6) //wss://<endere... => retirar wss://
...
export async function handler(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult> {
    ...
    try {
        ...
        if (invoiceTransaction.transactionStatus === InvoiceTransactionStatus.GENERATED){
            ...
            console.error(`Can´t cancel on ongoing process`)
        }
    } catch(error) {
