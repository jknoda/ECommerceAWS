...
const authInforService = new AuthInfoService(cognitoIdentityServiceProvider)
...
export async function handler(event: APIGatewayProxyEvent, context: Context): Promise<APIGatewayProxyResult>{
    ...
    const isAdmin = authInforService.isAdminUser(event.requestContext.authorizer)
    const authentcateUser = await authInforService.getUserInfor(event.requestContext.authorizer)
    ...
    if (method === "GET"){
        if (event.queryStringParameters){
            ...
            if (isAdmin || email === authentcateUser) {
                if (email) {
                    if (orderId) {
        ...
        if (!isAdmin){
            orderRequest.email = authentcateUser
        ...
        if (products.length === orderRequest.productIds.length){
            ...
            const results = await Promise.all([orderCreatedPromisse, eventResultPromisse]) // results 0 1
            ...
            console.error("Some product was not found") // capturado pelo alarm
