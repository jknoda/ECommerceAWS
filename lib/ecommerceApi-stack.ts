...
export class ECommerceApiStack extends cdk.Stack {
    ...
    private createCognitoAuth() {
        ...
        this.customerPool = new cognito.UserPool(this, "CustomerPool", {
            ...
            autoVerify: {
                email: true, // envia email para confirmação
                phone: false
            },
            userVerification: {
                emailSubject: "Verify your email for the ECommerce service!",
                emailBody: "Thanks for signing up to ECommerce servcie! Your verification code is {####}", // {####} para codigo de verificacao
        ...
        this.adminPool = new cognito.UserPool(this, "AdminPool", {
            ...
            userInvitation: {
                emailSubject: "Welcome to ECommerce adminstrator service",
                emailBody: "Your username is {username} and temporary password is {####}" // {..} variaveis que o sistema preenche
            },
            signInAliases: {
                username: false,
                email: true
        ...
        this.customerPool.addDomain("CustomerDomain", {
            cognitoDomain: {
                domainPrefix: "jkn-customer-service"
            }
        }) 

        this.adminPool.addDomain("AdminDomain", {
            cognitoDomain: {
                domainPrefix: "jkn-admin-service"
            }
        }) 
    ...
    private createOrdersService(props: EComerceApiStackProps, api: apigateway.RestApi) {
        ...
        // GET /orders       
        // GET /orders?email=teste@teste.com.br
        // GET /orders?email=teste@teste.com.br&orderId=123
        ...
        // DELETE /orders?email=teste@teste.com.br&orderId=123
        ...
        // GET /orders/events?email=test@teste.com.br
        // GET /orders/events?email=test@teste.com.br&eventType=ORDER_CREATED
    ...
    private createProductsService(props: EComerceApiStackProps, api: apigateway.RestApi) {
        const productsFetchIntegration = new apigateway.LambdaIntegration(props.productsFetchHandler);

        // autorizador usando const
        const productsFetchWebMobileIntegrationOption = {
            authorizer: this.productsAuthorizer,
            authorizationType: apigateway.AuthorizationType.COGNITO,
            authorizationScopes: ["customer/web","customer/mobile", "admin/web"] //ResourceServer/scope
        }

        const productsFetchWebIntegrationOption = {
            authorizer: this.productsAuthorizer,
            authorizationType: apigateway.AuthorizationType.COGNITO,
            authorizationScopes: ["customer/web", "admin/web"] //ResourceServer/scope
        }
        ...
        const productRequestValidator = new apigateway.RequestValidator(this, "ProductRequestValidator", {
            // autorizador direto no parametro
            restApi: api,
            requestValidatorName: "ProductRequesValidator",
            validateRequestBody: true
        })
